...
int dotState = 0;  // variabile per permettere il blinking dei due punti
                   // (utilizzatio per i secondi)
...
void setup() {
  lcd.begin(16, 2);
  // utilizzo attachInterrupt per usare una funzione creata per l'aumento delle
  // ore e dei minuti
  ...
  while (hours < ONE_DAY_IN_HOURS) {
    while (minutes < ONE_HOUR_IN_MINUTES) {
      while (seconds < ONE_MINUTE_IN_SECONDS) {
        ...
        } else {  // se invece è superiore a 10 vengono mostrati i minuti
                  // normalmente
          lcd.setCursor(3, 0);
          lcd.print(minutes);
        }
        if (hours < 10) {  // se le ore sono inferiori a 10 viene messo uno zero
                           // davanti all'ora per non visualizzarla come 1,
                           // 2, 3... ma come 01, 02, 03...
          lcd.setCursor(0, 0);
          lcd.print("0");
          lcd.print(hours);
        } else {  // se invece è superiore a 10 vengono mostrati i minuti
        ...
        seconds++;  // completato il cilco viene aumentata la variabile dei
                    // secondi.
      }
      seconds = 0;  // quando passano 60 secondi viene pulito il display e
                    // aumentata la variabile dei minuti
      lcd.clear();
      minutes++;
    }
    minutes = 0;  // passati 60 minuti si resetta la variabile della stessa e
                  // aumenta la variabile delle ore
    hours++;
  }
...
void minutesButtonFunc() {
  while (digitalRead(buttonUpMinutes) == 0) {
  ...
  if (minutes < 10 and hours > 10) {  // se i minuti sono minori a 10 e le ore maggiori di dieci
    ...
    lcd.print(":");
    lcd.print(minutes);
  } else {  // se inevece sono maggiori o uguali a 10 vengono mostrati
            // normalmente
    lcd.clear();
    lcd.print(hours);
    lcd.print(":");
...
void hoursButtonFunc() {
  while (digitalRead(buttonUpHours) == 0) {
  }  // il processo in questa funzione è analogo, ripetuto per il pulsante delle
     // ore.
  delay(500);
  hours++;
